...
record('Part 1 welcome renders', await page.getByRole('heading', { name: 'Building Clarity and Confidence' }).isVisible());
...
for (let day = 1; day <= 10; day += 1) {
  ...
  for (const id of coreIds) {
    await page.getByTestId('response-' + id).fill(
      'Day ' + day + ' response for ' + id + ': I identify the claim, evidence, assumptions, uncertainty, and what would make me revise the conclusion.'
    );
  }
  ...
  if (day === 5 || day === 10) {
    ...
    await page.getByTestId('checkpoint-response').fill(
      'Checkpoint after Day ' + day + ': the most useful change is making assumptions visible, testing evidence, and noticing when a reasoning shortcut is shaping the decision.'
    );
...
await page.getByTestId('final-reflection').fill(
  'My biggest Part 1 insight is that clearer reasoning requires me to separate claims, evidence, assumptions, and alternative explanations before I become confident.'
);
...
const stored = await page.evaluate(() => JSON.parse(localStorage.getItem('ctj.part1.state.v2') || '{}'));
...
record('Mobile welcome renders', await mobilePage.getByRole('heading', { name: 'Building Clarity and Confidence' }).isVisible());
