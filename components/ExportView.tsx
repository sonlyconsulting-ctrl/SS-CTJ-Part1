...
export const ExportView: React.FC = () => {
  ...
  const payload = {
    meta: {
      product: "The Critical Thinker's Journey Part 1: Building Clarity and Confidence",
      version: APP_VERSION,
  ...
  const buildText = () => {
    const lines: string[] = [
      ...
      'PART 1: BUILDING CLARITY AND CONFIDENCE',
      '',
  ...
  const handleTXT = () => downloadBlob(new Blob([buildText()], { type: 'text/plain;charset=utf-8' }), 'ctj-part1-responses.txt');
  const handleJSON = () => downloadBlob(new Blob([JSON.stringify(payload, null, 2)], { type: 'application/json;charset=utf-8' }), 'ctj-part1-responses.json');
  ...
  const handlePDF = async () => {
    ...
    write('PART 1: BUILDING CLARITY AND CONFIDENCE', 16, true);
    y += 3;
    ...
    doc.save('ctj-part1-responses.pdf');
  };
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-8 animate-fade-in" data-testid="export-view">
      ...
      <div className="text-center">
        <p className="text-xs uppercase tracking-[0.22em] text-ctj-gold">Part 1 Complete</p>
      ...
      <button
        onClick={() => {
          if (window.confirm('Start Part 1 again? Your current local responses will be cleared. Export first if you want to keep them.')) {
            dispatch({ type: 'RESET_WORK' });
          }
        }}
        className="btn-secondary w-full inline-flex justify-center items-center gap-2"
      >
        <RotateCcw size={18} /> Start Part 1 Again
      </button>
    </div>
  );
};
